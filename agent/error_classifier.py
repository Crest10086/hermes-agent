...
# pages (or lower max_seq_len / increase cache), never to delete conversation
...
def _kv_cache_capacity_verdict(
    error_msg: str,
    approx_tokens: int,
    context_length: int,
    result_fn,
    ...
    """
    if not any(p in error_msg for p in _KV_CACHE_CAPACITY_PATTERNS):
        return None
    # A genuinely large session may reflect a real overflow → keep compression.
    is_large = approx_tokens > context_length * 0.6 or (
        context_length <= 256000 and approx_tokens > 120000
    )
    if is_large:
        return None
    logger.info(
        "KV-cache capacity rejection (not context overflow): approx_tokens=%d "
        "context_length=%d reason=%s", approx_tokens, context_length,
...
def _classify_by_status(
    ...
    *,
    ...
    if status_code in {500, 502}:
        ...
        kv = _kv_cache_capacity_verdict(error_msg, approx_tokens, context_length, result_fn)
        if kv is not None:
            return kv
